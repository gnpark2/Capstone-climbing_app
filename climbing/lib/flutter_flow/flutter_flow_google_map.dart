...
//cluster와 아닌것 나누기까지 성공 cluster 성공
*/
...
class _FlutterFlowGoogleMapState extends State<FlutterFlowGoogleMap> {
  ...
  Future<Marker> Function(Cluster<FlutterFlowMarker>) get _markerBuilder =>
      (cluster) async {
        final markerImage = cluster.items.last.imageUrl;
        final newIcon = await createMarkerFromUrl(context, markerImage);
        print('여기 newIcon 이미지: ${newIcon}');
        print('여기 cluster contains: ${cluster.items.last.imageUrl} markers');
        print('여기 cluster contains: ${cluster.items.length} markers');
        return Marker(
          markerId: MarkerId(cluster.getId()),
          position: cluster.location,
...
//clustering 됨, image를 marker로 됨, marker를 계속 불러와서 깜빡거림이 있음. post할 때 이미지의 사이즈 조정이 필요할 것 같음 중구난방임.