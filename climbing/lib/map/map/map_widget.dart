                              ...
                              await showModalBottomSheet(
                                ...
                                builder: (context) {
                                  if(clusterDiff == "clustering") {
                                    //잘 들어감. 그런데 반복되는 동안 하나씩 들어감. 마지막에 Tap반응을 넣는게 맞을듯
                                    //passClusterMarkers.add(postRecord);
                                    //passClusterMarkers = googleMapPostRecordList.where((record) => record.latlng != null && record.latlng!.serialize() == postRecord.latlng!.serialize()).toList();
                                    passClusterMarkers = googleMapPostRecordList.where((record) => record.latlng != null && record.latlng!.serialize() == postRecord.latlng!.serialize()).toList();
                                    print("여기 뭐임? ${passClusterMarkers.length}");
                                    //
                                    return GestureDetector(
                                      onTap: () => _model.unfocusNode.canRequestFocus
                                          ? FocusScope.of(context).requestFocus(_model.unfocusNode)
                                      ...
                                      child: Padding(
                                        padding: MediaQuery.viewInsetsOf(context),
                                        child: MapMultiLinkPostWidget(
                                          upPost: passClusterMarkers,//list로 바꿈. 원래는 postRecord, map_multi_link_post_widget에서 List<PostRecord>를 PostRecord로.
                                        ),
                                      ),
                                    );
                                  } else {
                          ...
                          return FlutterFlowMarker(
                            ...
                            () async {
                              if (clusterDiff == "clustering") {
                                ...
                                print("Cluster markers count: ${passClusterMarkers.length}");
                              } else {
...
//cluster된 marker와 single marker에 대한 tap은 나눴지만 제대로된 기능x(cluster된 marker는 marker수만큼 위젯이 나타나며, post가 제대로 안넘어감.)
