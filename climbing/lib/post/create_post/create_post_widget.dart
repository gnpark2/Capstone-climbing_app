                  ...
                  children: [
                    Padding(
                      ...
                      child: FlutterFlowIconButton(
                        ...
                        onPressed: () async {
                          final selectedMedia = await ImagePicker().pickImage(
                            maxWidth: 1080.00,
                            maxHeight: 1080.00,
                            imageQuality: 100,
                            source: ImageSource.gallery,
                          );
                          if (selectedMedia != null && validateFileFormat(selectedMedia.path, context)) {
                            setState(() {
                              selecF = selectedMedia;
                              print(selecF);
                            });

                            setState(() => _model.isDataUploading1 = true);
                            dynamic selectedUploadedFile;
                            dynamic downloadUrl;

                            try {
                              selectedUploadedFile = FFUploadedFile(
                                name: selectedMedia.path.split('/').last,
                                bytes: await selectedMedia.readAsBytes(),
                                height: null, // You can get the dimensions using another package like image/image.dart
                                width: null,
                                blurHash: null,
                              );

                              downloadUrl = await uploadData(selectedMedia.path, await selectedMedia.readAsBytes());
                              print(downloadUrl);
                            } finally {
                              _model.isDataUploading1 = false;
                            }
